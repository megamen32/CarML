...
def save_model(actor_model, critic_model, params, filename="best_model.pth"):
    """Saves the actor and critic models along with hyperparameters."""
    torch.save({
...
def load_model(filename="best_model.pth"):
    """Loads the actor and critic models and their hyperparameters."""
    ...
    return loaded_actor, loaded_critic, checkpoint['hyperparameters']


def train(env, num_episodes=1000, gamma=0.99, actor_lr=0.1, critic_lr=0.01, tau=0.90,noise_std=0.5):
    state_dim = env.observation_space.shape[0]
    action_dim = env.action_space.shape[0]
    if os.path.exists('best_model.pth'):
        actor,critic,_=load_model()
    else:
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            with torch.no_grad():
                ...
                noise_cur=max(0.001,noise_std*(num_episodes//2-episode)/(num_episodes//2))
                noise = np.random.normal(0, noise_cur, size=action.shape)
                action += noise
        ...
        if average_reward > best_reward:
            ...
            best_params = params
            save_model(actor, critic, best_params)
    return actor,critic
...
for params in parameter_combinations:
    ...
    actor_model,critic = train(env, num_episodes=params[0], gamma=params[1], actor_lr=params[2], critic_lr=params[3], tau=params[4], noise_std=params[5])
